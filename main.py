...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            print("=> loaded checkpoint '{}' (epoch {})".format(args.resume, checkpoint['epoch']), flush=True)
    else:
    ...
    print(f"Size of the train set: {len(train_loader.dataset)}", flush=True)
    print(f"Size of the test set: {len(val_loader.dataset)}", flush=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_loss, train_prec1, train_prec5 = train(train_loader, model, criterion, optimizer, epoch, args.print_freq)

        print("validation!", flush=True)
        # evaluate on validation set
        val_loss, val_prec1, val_prec5 = validate(val_loader, model, criterion, args.print_freq)

        # Log training and validation metrics
        wandb.log({
            "epoch": epoch,
            "train_loss": train_loss,
            "train_prec@1": train_prec1,
            "train_prec@5": train_prec5,
            "val_loss": val_loss,
            "val_prec@1": val_prec1,
            "val_prec@5": val_prec5
        })
...
def validate(val_loader, model, criterion, print_freq):
    #print("in validation loop!", flush=True)
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    print(' * Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f}'.format(top1=top1, top5=top5), flush=True)
